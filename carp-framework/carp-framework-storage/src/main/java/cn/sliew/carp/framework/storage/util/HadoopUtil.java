 ...
 */
package cn.sliew.carp.framework.storage.util;
...
public enum HadoopUtil {
    ...
    public static Configuration getHadoopConfiguration(String hadoopConfPath) {
        ...
        for (String possibleHadoopConfPath : possibleHadoopConfPaths(hadoopConfPath)) {
            if (new File(possibleHadoopConfPath).exists()) {
                if (new File(possibleHadoopConfPath + "/" + HdfsConfigProperties.CORE_SITE_XML).exists()) {
                    retConf.addResource(
                            new org.apache.hadoop.fs.Path(
                                    possibleHadoopConfPath + "/" + HdfsConfigProperties.CORE_SITE_XML));

                    if (log.isDebugEnabled()) {
                        log.debug("Adding {}/{} to hadoop configuration",
                                possibleHadoopConfPath, HdfsConfigProperties.CORE_SITE_XML);
                    }
                }
                if (new File(possibleHadoopConfPath + "/" + HdfsConfigProperties.HDFS_SITE_XML).exists()) {
                    retConf.addResource(
                            new org.apache.hadoop.fs.Path(
                                    possibleHadoopConfPath + "/" + HdfsConfigProperties.HDFS_SITE_XML));
    ...
    public static String[] possibleHadoopConfPaths(String hadoopConfPath) {
        ...
        possiblePaths[1] = System.getenv(HdfsConfigProperties.HADOOP_CONF_DIR_ENV);

        if (System.getenv(HdfsConfigProperties.HADOOP_HOME_ENV) != null) {
            possiblePaths[2] = System.getenv(HdfsConfigProperties.HADOOP_HOME_ENV) + "/conf";
            possiblePaths[3] = System.getenv(HdfsConfigProperties.HADOOP_HOME_ENV) + "/etc/hadoop"; // hadoop 2.2
        }
