...
@Slf4j
public class JedisClusterQueue extends RedisQueue<JedisCluster> {

    private final JedisCluster jedisCluster;

    public JedisClusterQueue(JedisCluster jedisCluster, String queueName, ObjectMapper mapper, QueueExecutor queueExecutor, Collection<MessageHandler<?>> handlers, List<DeadMessageCallback> deadMessageHandlers, EventPublisher publisher, MeterRegistry meterRegistry, Boolean fillExecutorEachCycle, Duration requeueDelay, Duration requeueMaxJitter, Boolean canPollMany, TemporalAmount ackTimeout, Integer lockTtlSeconds) {
    ...
    public void cacheScript() {
        setReadMessageWithLockScriptSha(jedisCluster.scriptLoad(READ_MESSAGE_WITH_LOCK_SRC, getQueueKey()));
    }

    @Override
    protected <T> T withJedis(Function<JedisCluster, T> function) {
        return function.apply(jedisCluster);
    }

    protected <E extends Throwable> List<Object> multi(CheckedConsumer<Transaction, E> block) {
        try (Connection connection = jedisCluster.getConnectionFromSlot(JedisClusterCRC16.getSlot(getQueueKey()));
